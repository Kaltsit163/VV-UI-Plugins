...
.et-wrapper.et-x-left {
  right: auto;
  left: 0;
}

.et-wrapper.et-x-right {
  left: auto;
  right: 0;
}

.et-wrapper.et-x-center {
  left: 50%;
  transform: translateX(-50%);
}
.et-wrapper.et-y-center {
  top: 50%;
...
export default {
  ...
  computed: {
    ...
    clazz: function () {
      let clazz = []
      let className = this.mergedOption.className
      ...
      if (horizontalPosition) {
        clazz.push(`et-x-${horizontalPosition}`)
      }
      if (verticalPosition) {
        clazz.push(`et-y-${verticalPosition}`)
      }
      if (horizontalPosition === 'center' && verticalPosition === 'center') {
        clazz.push('et-center')
      }
      return clazz.join(' ')
    }
  },
